function [y,c] = adapt_simp ( f, a, b, TOL )
...
%
% Adapted from code written by Brian Bradie, bbradie@pcs.cnu.edu

fa = feval ( f, a );
fc = feval ( f, (a+b)/2 );
fb = feval ( f, b );
sab = (b-a)*(fa + 4*fc + fb)/6;
[approx eest nf c] = as ( sab, fa, fc, fb, f, a, b, TOL );
...
function [app, est, nf, c] = as ( sab, fa, fc, fb, f, a, b, TOL )
...
if ( errest < (10.0*TOL) )
   ...
   [a1 e1 n1 c1] = as ( sac, fa, fd, fc, f, a, c, TOL/2.0 );
   [a2 e2 n2 c2] = as ( scb, fc, fe, fb, f, c, b, TOL/2.0 );
   app = a1 + a2;
   est = e1 + e2;
   nf = n1 + n2 + 2;
   c = [c,c1,c2];
